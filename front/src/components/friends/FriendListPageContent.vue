...
const wumpusSentence = ["Le Wumpus est toujours en ligne!", "Le Wumpus sera toujours ton ami", "Personne attends le Wumpus", "Tu ne peux pas bloquer le Wumpus woula."]
...
<template>
  <div v-if="friendList.length === 0" class="w-full h-full flex flex-col justify-center items-center">
    ...
    <span class="text-[16px] leading-5 text-center text-white-100">{{ wumpusSentence[friendsStore.getDisplayed()]
    }}</span>
  </div>
  ...
  <div class="overflow-x-hidden overflow-y-scroll pr-0 pb-2 mt-2 relative box-border min-h-0 grow shrink basis-auto">
    <div class="relative">
      <UserCardList v-for="friend in friendList" :receiverId="friend.receiver" :initiatorId="friend.initiator" />
    </div>
  </div>
</template>
