...
<template>
  <div class="absolute top-0 left-0 w-screen h-screen bg-grey-100/70 z-30 flex justify-center items-center">
    <div ref="target" class="relative w-[660px] bg-[#1e1f22] rounded flex flex-col items-center p-4">
      <div class="p-4">
        <h2 v-if="inviteExpired" class="text-white-500 text-2xl font-bold">
          Mince ! L'invitation à expiré ou à déja trop été utilisé !
        </h2>
        <h2 v-else class="text-white-500 text-2xl font-bold">Voulez vous rejoindre le serveur "{{ serverName }}"</h2>
